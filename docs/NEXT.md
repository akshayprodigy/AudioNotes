...
> were on this list until 12-13 September; all three are closed, see items 9, 10 and 13.

**Two decisions this list is built on:**

1. **v1 is English only.** Already the state of the code. Hindi and Qwen3-ASR stay built and
   unreachable until a language has a word-error-rate number behind it. **Reaffirmed and final,
   12 September 2026** — not open for re-raising before launch.
...
**Settled 12 September 2026 — do not re-ask:**

- **Crash reporting is Firebase Crashlytics, not Sentry.** The code already says so: it moved on
  7 September (`00f08a4`) and has been on `main` since `35abf1b`. There is no DSN to send and no
  account to create — `android/app/google-services.json` is committed. It ships off until the user
  says yes, debug builds never report, and the privacy screen names Crashlytics as the one source
  it cannot count (`src/telemetry/crash.ts`). Item 10 below is closed on this.
- **Hosting has enough bandwidth.** The Hostinger ceiling is not a number anyone is chasing any
  more. Item 13 below is closed on this.
- **Prices are final: ₹299 / month, ₹2,499 / year in India; $4.99 / month, $39.99 / year
  elsewhere. Two tiers, Free and Pro. No lifetime SKU.** Decided by 7 September and already
  public on the landing page (`server/app/landing.py`); the billing test fixture carries the same
  numbers as "the founder's launch prices". The app does not hard-code them — the paywall reads
  whatever Play reports, which is why item 9 below is now about Play Console, not about numbers.
...
|---|---|---|---|---|
| 1 | ~~Smoke-test the current build on a phone~~ **DONE 6 Sep** | me | S | Ran end to end on the Pixel 7 Pro: onboarding, record, VAD, whisper, diarization, transcript, auto-title, export path and Settings, plus 16 native instrumentation tests with zero skips — the first genuinely green device run this project has had. Was still the top technical risk. The four native changes it was waiting on — the Sentry Gradle plugin (since replaced by Crashlytics, see item 10), the engine factory, the Qwen engine and the detection rewrite — are all now exercised on hardware. |
| 2 | **Two more mid-range non-Pixel phones, end to end** — *A07 re-run on the release build 14 Sep: passed end to end, found three more* | you + me | M | **14 Sep, Galaxy A07, release build:** record → screen off → stop → processed with the screen off in 49 s for a 99 s meeting, Samsung's memory killer reaping other apps around it — background processing holds on One UI 8.5 with the battery exemption granted. Tap-to-play, export stamps and a tick surviving Redo all verified. It also found three more bugs, none Samsung-specific: every text prompt had lost its Cancel/Save buttons (fixed), Crashlytics collected before consent via react-native-firebase's own manifest key (fixed, guarded), and consent read "on" after first-run onboarding with nothing tapped (open, needs a wipeable device). Two extractor misses on textbook sentences — "we have decided" and "<Name> will <verb> by Friday" — are recorded in the spec's §14. **Evening session, same phone, wiped, volume up:** pause/resume, PiP, stop-from-notification and headless processing all passed — and the consent clip turned out to be **eating the first half-minute of every meeting** on any phone not on silent (the decision and both actions were gone; the audio had them; reproduced on the host). Fixed with the clip's span kept out of whisper (`71ec6a2`), plus the notes-ready notification no longer opening the paywall, the two extractor misses, and "AudioNotes" in the notification. On-device proof of those four still owed. **Late 14 Sep, Pixel 7 Pro, Android 17:** Task 14 closed on the real library (ticks survive Redo on rule-extracted actions), the four evening fixes proven on a second phone, and the full feature pass run — everything in the spec's §14.2 table passed except what is marked not exercised; five more defects fixed on the spot (hidden recording notification on Android 17, stale speaker label, SRT export unreachable, and two smaller). **The one thing that does not work is buying: no `verbale_pro` product answers.** Still owed: a Redmi/Realme with MIUI/ColorOS. The A07 found three bugs in one meeting. Redmi, Realme or Samsung A-series. MIUI, ColorOS and Funtouch also kill `dataSync` foreground services aggressively, which is what background processing, headless minutes and the "Notes ready" notification all depend on. A meeting that dies mid-recording on a Redmi is a user who never gets their notes and never comes back. |
...
| 9 | ~~Final prices~~ **DECIDED — what is left is the Play Console product** | you + me, ~1 hr | — | ₹299 / ₹2,499 in India, $4.99 / $39.99 elsewhere; see the Settled block above. The plans screen this was said to block **already shipped 7 Sep** (`42fbda2`, `810c279`): one subscription `verbale_pro` with two base plans, chosen by name, annual preselected, and a strikethrough drawn only from twelve times the monthly rate or Play's own full price — never a constant. Per-plan model access is moot with two tiers; the existing yes/no gate is the design. **What nothing in this repo can confirm:** that `verbale_pro` exists in Play Console with base plans `monthly` and `annual` at those prices. The paywall shows no number until it does — it reads prices live from Play — so no device run has ever seen a real one. Create the product, set INR and USD by hand, let Play convert the rest unless a market needs a hand-set figure, then put a licensed tester through the paywall on a phone and note the result here. **Found 22 Sep, and it is a blocker on its own:** the live server has Play Billing unconfigured — `POST /api/billing/play/link` answers `503 Play Billing is not configured on this server` because `PLAY_SERVICE_ACCOUNT_JSON` is unset in `/opt/verbale/.env`. On that answer the app acknowledges the purchase locally and stores no licence, so a buyer would pay and get nothing. A Google Cloud service account with the Android Publisher API, invited to Play Console with *Manage orders and subscriptions*, its JSON on the VPS — before any tester buys. The probe and the steps are in `docs/play-console.md`. |
| 10 | ~~Sentry DSN~~ **CLOSED 12 Sep — Firebase Crashlytics, not Sentry** | — | — | Decided, and already in the code since 7 Sep (`00f08a4`, on `main` from `35abf1b`). Nothing to send: `android/app/google-services.json` is committed, so there is no "not configured" state. Still ships off until the user consents, and debug builds never report. The reason it exists is unchanged — launching on unfamiliar hardware without crash reports is launching blind, which items 1 and 2 say is the actual risk — but the founder's 15 minutes are no longer needed. **Do not open a Sentry account.** |
...
| 12 | ~~Deploy the website~~ **DONE — live at verbale.innocorelabs.com** | — | — | Deployed with the licence server (31 Aug); the landing page, `/privacy`, `/terms` and `/delete-account` answer, and the copy on the live site matches `server/app/landing.py` as of 22 Sep. The model mirror on the same host has NOT been re-run since the meaning index was added to the catalog (16 Sep) — `bge-small-en-v1.5-q8_0.gguf` is 404 there and first-run falls back to Hugging Face for it; `server/deploy/mirror-models.sh` fixes that in one idempotent run (your word, it is a deploy). |
| 13 | ~~Hostinger bandwidth ceiling~~ **CLOSED 12 Sep — hosting has enough** | — | — | Founder confirmed the hosting plan's bandwidth covers launch. No number to chase. (For the record, Pro is about 1.1 GB per install rather than 2.2 GB, and a free install is 114 MB.) |
