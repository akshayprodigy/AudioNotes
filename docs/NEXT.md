...
| 3 | ~~Measure Parakeet-TDT and Moonshine v2 against whisper-base~~ **DONE 5 Sep — do not swap** | me | S | **Parakeet-TDT 20.2% WER against whisper's 29.0% on identical audio, and it cannot ship.** It returns an empty string on quiet windows and reports success — 60% of windows lost on two of four AMI meetings, ×3 gain recovers them, sherpa logs no error. Also 661 MB against 57 MB, 1.7 GB peak RSS, 2.4-2.9x the ASR time. Moonshine 27.2%, no cliff, not worth 5x the download for 6%. Full numbers and what to check next: `docs/superpowers/eval-english-engine-candidates.md`. |
| 4 | **Build an accent-diverse English test set** | you + me, half a day | S | **Now the top accuracy item, since item 3 says the engine is not changing before launch.** **The gap nobody has named yet.** The only English benchmark here is AMI: British and European meeting speech recorded in 2005 on headset mics. A global launch sells to American, Australian, Indian, Singaporean and Scottish speakers, and there is currently zero measurement on any of them. Three or four short real recordings with corrected transcripts is enough to stop flying blind. |
