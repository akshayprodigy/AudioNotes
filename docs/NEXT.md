...
| 3 | **Measure Parakeet-TDT and Moonshine v2 against whisper-base** | me, half a day | S | English is the whole product. whisper-base is 29.7% on AMI; Parakeet publishes about 16% on the sherpa-onnx runtime already vendored, behind the `AsrEngine` factory that already exists. If it holds, one model swap roughly halves the error rate of everything being sold. Know before the listing, not after. |
| 4 | **Build an accent-diverse English test set** | you + me, half a day | S | **The gap nobody has named yet.** The only English benchmark here is AMI: British and European meeting speech recorded in 2005 on headset mics. A global launch sells to American, Australian, Indian, Singaporean and Scottish speakers, and there is currently zero measurement on any of them. Three or four short real recordings with corrected transcripts is enough to stop flying blind. |
...
| 6 | **Give the refused meeting a next step** | me, 1 day | S | English-only means more people meet "NOT ENGLISH". The audio is already kept for exactly this. It needs to say so and offer to re-run when the language lands, or it reads as a bug. Research §2.4 #15. |
| 7 | **The consent kit** | me, 2 days | S | **Bigger for a global launch than an Indian one.** A spoken announcement, a card to show the room, and region-aware defaults. All-party-consent US states, GDPR in Europe. Otter is in a consolidated wiretap class action over exactly this, and it is the cheapest possible way to turn their liability into a selling point. |
