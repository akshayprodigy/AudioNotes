...
MUTATIONS = [
    ("confident match returns a fresh uuid",
     ...
     "      rows.add(Row(UUID.randomUUID().toString(), incoming[i], review, old.createdAt, null, old.record))"),
    ("matched row keeps the stored text instead of the new one",
     ...
     " old.anchorStartMs, old.anchorEndMs), review, old.createdAt, null, old.record))"),
    ("created_at is not carried through a match",
     "      rows.add(Row(old.id, incoming[i], review, old.createdAt, null, old.record))",
     "      rows.add(Row(old.id, incoming[i], review, null, null, old.record))"),
    # Phase B: the typed record rides the match. Without it a reprocess silently un-classifies.
    ("the typed record is dropped on a match",
     "      rows.add(Row(old.id, incoming[i], review, old.createdAt, null, old.record))",
    ...
    ("preserved rows are relabelled with the run's gen_version",
     ...
     "      rows.add(Row(old.id, preserved, review, old.createdAt, null, old.record))"),
    ("the typed record is dropped on a preserved row",
     "      rows.add(Row(old.id, preserved, review, old.createdAt, old.genVersion, old.record))",
