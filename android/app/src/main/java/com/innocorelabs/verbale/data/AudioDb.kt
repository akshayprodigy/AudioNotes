...
import com.innocorelabs.verbale.pipeline.PeopleMatch
...
class AudioDb private constructor(private val db: SQLiteDatabase) {
  ...
  /**
    ...
    * in, and `samples` is the old count. The new centroid is normalise(old * samples + new), which
  ...
  /**
    ...
    * `dim` is passed separately (extracted by the caller from the same array). An all-zero row
  ...
  /**
    ...
    * The checks run in order: setting off → off; not entitled → not_pro; name blank → blank; name
    * looks like "Speaker N" → default_name; speaker has no voice → no_voice. Otherwise the person is
